...
// Inlines the rmt_set_tx_intr_en function to avoid the overhead of a function
// call
...
// ignore warnings like: ignoring attribute 'section (".iram1.11")' because it
// conflicts with previous 'section (".iram1.2")' [-Wattributes] #pragma GCC
// diagnostic ignored "-Wattributes"
...
#define FASTLED_DEBUG(format, errcode, ...)                                    \
    FASTLED_ASSERT(format, errcode, ##__VA_ARGS__)
#else
...
//    interrupt to fire too often. When combined with WiFi, this leads to
//    conflicts between interrupts and weird flashy effects on the LEDs. Special
//    thanks to Brian Bulkowski for finding this problem and developing a fix.
...
// This is work-around for the issue of random fastLed freezes randomly
// sometimes minutes but usually hours after the start, probably caused by
// interrupts being swallowed by the system so that the gTX_sem semaphore is
// never released by the RMT interrupt handler causing FastLED.Show() never to
// return.
...
// For example: To wait a maximum of two seconds (enough time for the Esp32 to
// have sorted itself out and fast enough that it probably won't be greatly
// noticed by the audience) use: # define FASTLED_RMT_MAX_TICKS_FOR_GTX_SEM
// (2000/portTICK_PERIOD_MS) (Place this in your code directly before the first
// call to FastLED.h)
...
#ifndef F_CPU_RMT_CLOCK_MANUALLY_DEFINED // if user has not defined it manually
                                         // then...
...
// -- Convert ESP32 CPU cycles to RMT device cycles, taking into account the
// divider RMT Clock is typically APB CLK, which is 80MHz on most devices, but
// 40MHz on ESP32-H2 and ESP32-C6
...
//    out: A pointer into an array at least 8 units long (one unit for each
//    bit).
FASTLED_FORCE_INLINE void IRAM_ATTR convert_byte_to_rmt(
    FASTLED_REGISTER uint8_t byteval, FASTLED_REGISTER uint32_t zero,
    FASTLED_REGISTER uint32_t one, volatile rmt_item32_t *out) {
    FASTLED_REGISTER uint32_t pixel_u32 = byteval;
    pixel_u32 <<= 24;
    uint32_t tmp[8];
    for (FASTLED_REGISTER uint32_t j = 0; j < 8; j++) {
        FASTLED_REGISTER uint32_t new_val =
            (pixel_u32 & 0x80000000L) ? one : zero;
        pixel_u32 <<= 1;
...
ESP32RMTController::ESP32RMTController(int DATA_PIN, int T1, int T2, int T3,
    ...
    : mPixelData(0), mSize(0), mCur(0), mBufSize(0), mWhichHalf(0), mBuffer(0),
      mBufferSize(0), mCurPulse(0), mBuiltInDriver(built_in_driver) {
...
void ESP32RMTController::init(gpio_num_t pin, bool built_in_driver) {
    ...
    for (int i = 0; i < ESP32RMTController::gMaxChannel;
         i += ESP32RMTController::gMemBlocks) {
        gOnChannel[i] = NULL;
    ...
    if (!built_in_driver) {
        ...
        if (gRMT_intr_handle == NULL) {
            esp_intr_alloc(
                ETS_RMT_INTR_SOURCE, ESP_INTR_FLAG_IRAM | ESP_INTR_FLAG_LEVEL3,
                ESP32RMTController::interruptHandler, 0, &gRMT_intr_handle);
        }
    }
...
void ESP32RMTController::startOnChannel(int channel) {
    ...
    if (mBuiltInDriver) {
        ...
        // -- Fill both halves of the RMT buffer (a totaly of 64 bits of pixel
        // data)
        fillNext(false);
        fillNext(false);
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
    FASTLED_DEBUG(false,
                  "tx_start Not yet implemented for ESP32-S2 in idf 5.x");
#else
    // rmt_ll_tx_reset_pointer(&RMT, mRMT_channel)
...
void IRAM_ATTR _rmt_set_tx_intr_disable(rmt_channel_t channel) {
    ...
    // Inline the code for rmt_set_tx_intr_en(channel, false) and rmt_tx_stop,
    // so it can be placed in IRAM
#if CONFIG_IDF_TARGET_ESP32C3
    // rmt_ll_enable_tx_end_interrupt(&RMT, channel)
...
#elif CONFIG_IDF_TARGET_ESP32S2
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
    FASTLED_ASSERT(false,
                   "doneOnChannel not yet implemented for ESP32-S2 in idf 5.x");
#else
    // rmt_ll_enable_tx_end_interrupt(&RMT, channel)
...
void IRAM_ATTR ESP32RMTController::doneOnChannel(rmt_channel_t channel,
                                                 void *arg) {
    ...
    if (check_time) {
        if (mLastFill != 0) {
            ...
            if (delta > (int32_t)mMaxCyclesPerFill) {
                ...
                /** -- Go back to the original strategy of just setting mCur =
                mSize and letting the regular 'stop' process happen
                * mRMT_mem_start = 0;
                RMT.int_ena.val &= ~(1 << (mRMT_channel * 3));
